...
export default class CanvasDraw extends PureComponent {
  ...
  getDataURL = (fileType, useBgImage, backgroundColour) => {
    ...
    console.log("fileType @=",fileType,"useBgImage@=", useBgImage,"backgroundColour@=", backgroundColour)
  ...
  clampPointToDocument = (point) => {
    console.log("clampPointToDocument")
  ...
  redrawImage = () => {
    ...
    this.image &&
      this.image.complete &&
      drawImageProp({ ctx: this.ctx.grid, img: this.image });
  };
  ...
  clearExceptErasedLines = () => {
    console.log("clearExceptErasedLines")
    this.lines = [];
    this.valuesChanged = true;
